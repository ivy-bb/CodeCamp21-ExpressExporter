...
>Proto Us0 Us0 ExportedUserTaskFormProcess #zField
Us0 f0 guid 162F0A4FA20C17F1 #txt
Us0 f0 method start(gawfs.ExecutePredefinedWorkflowData) #txt
Us0 f0 inParameterDecl '<gawfs.ExecutePredefinedWorkflowData execdata> param;' #txt
Us0 f0 inParameterMapAction 'out.executePredefinedWorkflowData=param.execdata;
' #txt
Us0 f0 outParameterDecl '<gawfs.ExecutePredefinedWorkflowData execdata> result;' #txt
Us0 f0 outParameterMapAction 'result.execdata=in.executePredefinedWorkflowData;
' #txt
...
' #txt
Us0 f6 actionCode 'import ch.ivy.gawfs.ExpressProcessUtils;
...
ExpressTaskDefinition expressTaskDefinition = BusinessEntityConverter.jsonValueToEntity(ivy.task.customFields().stringField("dynaform").getOrDefault("").replace("''", """"), ExpressTaskDefinition.class) as ExpressTaskDefinition;
taskDef = ExpressProcessUtils.initializeControllersForTaskDef(taskDef, expressTaskDefinition);

taskDef.actualApplicant = ivy.wf.getSecurityContext().findUser(taskDef.actualApplicantName);
in.executePredefinedWorkflowData.currentTask = taskDef;
in.executePredefinedWorkflowData.currentTask.dynaFormController.createForm();

in.executePredefinedWorkflowData.steps = ivy.case.customFields().stringField("steps").getOrDefault("").split(",");
in.executePredefinedWorkflowData.actualStepIndex = ivy.task.customFields().numberField("stepindex").getOrDefault(0);


// we have to reiinit the actualApplicant because IUser can not be serialized and are lost after a taskswitch
...
in.executePredefinedWorkflowData.currentTask.actualApplicantName = ivy.session.getSessionUserName();
in.executePredefinedWorkflowData.currentTask.actualApplicant = ivy.session.getSessionUser();
in.executePredefinedWorkflowData.currentTask.subject = ivy.task.getName();

in.executePredefinedWorkflowData.finishedTasks.add(in.executePredefinedWorkflowData.currentTask);
in.executePredefinedWorkflowData.currentTask = null;
...
>Proto Us0 .type ch.ivy.gawfs.workflowExecution.ExportedUserTaskForm.ExportedUserTaskFormData #txt
